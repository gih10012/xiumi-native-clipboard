...
4. For a document with embedded images, have the user click “① 复制全部图片（无格式）” and press a real `Ctrl+C` in the preview page. In `text/html`, follow every embedded real-image block with a unique transparent 1×1 Base64 marker block; leave already-persistent HTTP(S) images unpaired. Also include an empty/invisible `text/plain` fallback and no article text, layout, or Xiumi custom MIME. Xiumi currently reuses a stateful global Base64 regular expression while scanning sources, causing success/failure to alternate; each marker must absorb the failed scan immediately after its real Base64 image. The tool must discard those markers when the Xiumi body returns. Paste the sheet into a blank temporary Xiumi article.
...
The manual browser workflow is supported on desktop Windows, macOS, and Linux in current Edge/Chrome/Chromium. On Linux, the localhost② button can use `wl-copy` or `xclip`; on Windows and macOS, let that button fall back to a real `Ctrl+C` in the page. Do not claim support for mobile browsers, Firefox, or Safari because their custom clipboard behavior is outside this delivery contract.
