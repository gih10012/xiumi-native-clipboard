...
4. Have the user click “① 复制带格式 HTML” and press a real `Ctrl+C` in the preview page. This clipboard write must contain all article content as `text/html` plus `text/plain`, and no Xiumi custom MIME. Its top-level HTML blocks must each contain text or images, never both: Xiumi's HTML converter otherwise takes the text branch and silently drops nested images. Pasting it once lets Xiumi upload Base64 images and creates the default saveable result; complex native layouts may be flattened.
5. If the HTML result is satisfactory, ask the user to verify that it saves and stop. Native replacement is optional.
6. For optional native replacement, have the user wait for Xiumi's paste/upload indicator to finish, select and copy the imported Xiumi body, return to the tool, and press `Ctrl+V`. Xiumi uploads Base64 images serially, so visible images do not prove completion. The tool must distinguish pending embedded images from a partial body selection and use only a complete set of returned permanent URLs to unlock “② 复制 xiumi-comps（可选）”.
