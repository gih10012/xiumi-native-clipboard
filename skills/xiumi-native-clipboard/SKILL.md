...
4. For a document with embedded images, have the user click “① 复制全部图片（无格式）” and press a real `Ctrl+C` in the preview page. This clipboard write must contain only one image per top-level HTML block as `text/html`, an empty/invisible `text/plain` fallback, and no article text, layout, or Xiumi custom MIME. Paste it into a blank temporary Xiumi article.
