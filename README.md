...
3. 点击“① 复制带格式 HTML”，在页面内真实按下 `Ctrl+C`，再到秀米按 `Ctrl+V`。全部文字、图片和基础文字样式会一起导入；等图片加载完成后，这一版已经可以直接保存。复杂横排、出框与嵌套布局可能被秀米拍平。
...
①只写入 `text/html` 与 `text/plain`，因此秀米会执行普通 HTML 粘贴并上传 Base64 图片。为适配秀米的 HTML 转组件规则，①把正文拆成互不混合的顶层文字块和图片块，保证图片完整，复杂布局留给②恢复。②使用 Chromium DataTransfer 自定义格式；因为其中的图片地址已经从第一步的秀米正文回传，所以能够保存。本地服务的按钮为②提供等价的系统剪切板桥接。目标浏览器为桌面版 Edge/Chrome；Firefox 和 Safari 不在兼容范围内。
